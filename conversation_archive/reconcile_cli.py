"""CLI wiring for explicit, local review and installation steps."""
...
def add_parser(sub):
    parent = sub.add_parser(
        "reconcile", help="Prepare, review, and reconcile original conversations"
    )
    ...
    for command in ("packet", "record", "status", "check", "apply"):
...
def dispatch(args):
    ...
    if command == "prepare":
        ...
        if args.output:
            ...
            (
                r.atomic_text(args.output, render_packet(value))
                if args.format == "markdown"
                else r.atomic_json(args.output, value)
            )
            return {
                "status": "packet_generated_not_reviewed",
...
def render_packet(value):
    ...
    lines = [
        ...
        (
            f"Exported selected node: `{selected}`"
            if selected
            else "Exported selected branch: unknown."
        ),
        "",
        ...
        "",
        value["range_convention"],
        "",
        "## Exported branch structure",
        "",
    ]
    for n in c["graph"]:
        if n["parent_id"] not in nodes:
            lines.append(
                f"- Root `{n['node_id']}`; exported parent `{n['parent_id']}`; message `{n['message_record_id']}`"
            )
        if len(children[n["node_id"]]) > 1:
            lines.append(
                f"- Branch point `{n['node_id']}` → "
                + ", ".join(f"`{x}`" for x in children[n["node_id"]])
            )
    for piece in value["pieces"]:
        ...
        branch = (
            ...
            if selected is None
                ...
                "on exported selected path"
                if piece.get("node_id") in selected_path
                else "outside exported selected path"
            )
        )
        lines.extend(
            [
        ...
        if piece["attachments"]:
            lines.extend(
                [
                    "",
                    "Attachment references (availability does not prove the media was inspected):",
                    "",
